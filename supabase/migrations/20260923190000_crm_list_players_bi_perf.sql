-- BI: crm_list_players estava ~3s+ e estourava statement_timeout=8s do authenticated
-- sob carga. Remove scans pesados (table_details + DISTINCT ON em todas as TX)
-- e sobe o timeout da função.
...
BEGIN
  ...
  -- Só TX de incentivo (bônus ou sender = MKT GT), não as 67k linhas.
  mkt_incentives AS (
    SELECT
  ...
  -- Sem table_details (162k) nem scan completo de TX.
  player_ids AS (
  ...
  base AS (
    SELECT
      ...
      coalesce(m.nickname, lp.nickname, mi.nickname) AS nickname,
      lp.agent_id AS current_agent_id,
    ...
    LEFT JOIN agents a ON a.agent_id = lp.agent_id
...
ANALYZE public.campaign_transactions;
ANALYZE public.campaign_player_periods;
ANALYZE public.campaign_players;
