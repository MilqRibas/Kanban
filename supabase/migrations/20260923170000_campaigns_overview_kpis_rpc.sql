-- Snapshot de KPIs do board sem baixar períodos no client.
-- Aproximação alinhada ao overview: rake SX por agent_id, bônus SX por agent_id,
-- ativos = players com weekly_rake > 0 nos períodos SX do agente.
...
REVOKE ALL ON FUNCTION public.crm_campaigns_overview_kpis(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.crm_campaigns_overview_kpis(text) TO authenticated;
